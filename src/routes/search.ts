...
export function registerSearchRoutes(app: Hono) {
  ...
  app.get('/api/search', async (c) => {
    ...
    const allProjects = c.req.query('all_projects') === 'true'; // Cross-oracle search

    try {
      const result = await handleSearch(sanitizedQ, type, limit, offset, mode, project, cwd, model, allProjects);
