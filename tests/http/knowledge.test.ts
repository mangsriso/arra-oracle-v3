 ...
 * Covers src/routes/{search,knowledge,supersede}.ts. Every run owns a dedicated
 * server, port, HOME, repo root, data directory, and SQLite database under /tmp.
 */
...
describe("HTTP Contract — search / knowledge / supersede", () => {
  beforeAll(async () => {
    fixture = await startIsolatedHttpServer("oracle-knowledge-http");
  ...
  describe("GET /api/similar", () => {
    ...
    test("404-shaped payload for unknown id", async () => {
      ...
      // The hermetic test runtime disables the local vector service. 503 is
      // the explicit unavailable contract; it must never fall through to a
      // production vector endpoint.
